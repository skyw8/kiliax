...
impl LlmClient {
    pub fn new(route: ProviderRoute) -> Self {
        let provider = match route.api.clone() {
...
mod tests {
    ...
    #[test]
    fn provider_api_serializes_new_config_names() {
        let raw = serde_json::to_string(&ProviderApi::OpenAiChatCompletions).unwrap();
        assert_eq!(raw, "\"openai_chat_completions\"");
    ...
    #[test]
    fn provider_api_accepts_legacy_names() {
        let api: ProviderApi = serde_json::from_str("\"openai-compatible\"").unwrap();
        assert_eq!(api, ProviderApi::OpenAiChatCompletions);

        let api: ProviderApi = serde_json::from_str("\"anthropic\"").unwrap();
